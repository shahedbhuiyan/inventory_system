	...
	$.fn.extend({
		...
		UISugestion: function(options) {
			...
			$(this).focus(function() {
				...
				var drug = (defaults.parent_element == 'cname1') ? "drug1":"drug";
		...
		UISugestionItem: function(stext) {
			...
			for(i=0; i<defaults.itemList.length; i++) {
				...
				var id = defaults.itemList[i].id;
				
				itext = itext.toLowerCase();
				stext = stext.toLowerCase();
				//console.log(itext+"---"+stext);
				if(itext.indexOf(stext)==0 && itemLimit<=10) {
					$("<li value = "+id+">"+defaults.itemList[i].txt+"</li>").css({"width":$("#"+defaults.parent_element).width()-5+"px"})
				...
				if(key==38 || key==40) {
					...
					$("."+defaults.toggleId).find('li').each(function(i){
						if(defaults.currentSelected==i) {
							...
							getValueFromGrid($(this).attr('value'),defaults.parent_element);
						}
					});
					e.preventDefault();
				}
...
function getValueFromGrid(val,id_o){
	var id = id_o.slice(15);
	
	if(id_o == 'cname1'){
		$.ajax({
			type:'POST',
			url:"invoice_sql.php",
			data:{id_d:val,action:'getInfo'},
	...
	$.ajax({
		type:"POST",
		url:"invoice_sql.php",
		data:{id_d:val,action:'getPrice'},
		success:function(resp_val){
			var ary = resp_val.split("|");
			$("#unit_"+id).val(ary[0]);	
			$("#purchase_"+id).val(ary[1]);
			$("#item_"+id).val(ary[2]);
			$("#desc_"+id).val(ary[3]);
		}	
	});
