	...
	$.fn.extend({
		...
		UISugestion: function(options) {
			...
			$(this).focus(function() {
				...
				var drug //= (defaults.parent_element == 'iname') ? "iname":"";
				if(defaults.parent_element == 'iname'){
					drug = 'iname';
				} else if(defaults.parent_element == 'bname'){
					drug = 'bname';
				} else if(defaults.parent_element == 'cata'){
					drug = 'cata';
				} else if(defaults.parent_element == 'item'){
					drug = 'item';
				} else if(defaults.parent_element == 'customer'){
					drug = 'customer';
				}
				...
				if(key==38 || key==40) {
					...
					$("."+defaults.toggleId).find('li').each(function(i){
						...
						if(defaults.currentSelected==i) {
							...
							//getValueFromGrid($("#invoice_no").val(),defaults.parent_element);
